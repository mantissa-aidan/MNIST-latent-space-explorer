...
cur_DIGIT = np.zeros((3, input_h, input_w), dtype=np.uint8)
...
DIGIT_surface_mini = pygame.Surface((input_w, input_h))
DIGIT_surface = screen.subsurface((drawing_x, drawing_y, drawing_w, drawing_h))
pygame.display.set_caption('mnist PCA explorer')
...
def draw_DIGIT():
	pygame.surfarray.blit_array(DIGIT_surface_mini, np.transpose(cur_DIGIT, (2, 1, 0)))
	pygame.transform.scale(DIGIT_surface_mini, (drawing_w, drawing_h), DIGIT_surface)
...
while running:
	...
	if needs_update:
		...
		y = enc([x, 0])[0][0]
		cur_DIGIT = (y * 255.0).astype(np.uint8)
		needs_update = False
