...
PARAM_SIZE = 80
LR = 0.001
NUM_RAND_DIGITS = BATCH_SIZE
NUM_TEST_DIGITS = BATCH_SIZE
...
if CONTINUE_TRAIN:
	...
	print ("0 activation:",model.output_shape)
	assert(model.output_shape == (None, PARAM_SIZE))
	
	model.add(Reshape((PARAM_SIZE, 1, 1), name='encoder'))
	print ("1 activation:",model.output_shape)
	
	model.add(Conv2DTranspose(256, (4, 4)))           #(4, 4)
	model.add(Activation("relu"))
	print ("2 activation:",model.output_shape)

	model.add(Conv2DTranspose(128, 7))                #(8, 8)
	model.add(Activation("relu"))
	print ("3 activation:",model.output_shape)
	
	model.add(Conv2DTranspose(64, 5))                #(12, 12)
	model.add(Activation("relu"))
	print ("4 activation:",model.output_shape)

	model.add(Conv2DTranspose(3, (2,2), strides=2))      #(192, 144)
	model.add(Activation("sigmoid"))
	print ("8 activation:",model.output_shape)
...
rand_vecs = np.random.normal(0.0, 1.0, (NUM_RAND_DIGITS, PARAM_SIZE))

def make_rand_DIGITs(rand_vecs, iters):
	...
	y_DIGITs = func([x_vecs, 0])[0]
	for i in range(y_DIGITs.shape[0]):
		save_image(y_DIGITs[i], 'rand' + str(i) + '.png')
		if i < 5 and (iters % 10) == 0:
			if not os.path.exists('morph' + str(i)):
				os.makedirs('morph' + str(i))
			save_image(y_DIGITs[i], 'morph' + str(i) + '/img' + str(iters) + '.png')

make_rand_DIGITs(rand_vecs, 0)
...
for iters in range(NUM_EPOCHS):
	...
	y_DIGITs = model.predict(x_train[:NUM_TEST_DIGITS], batch_size=BATCH_SIZE)
	for i in range(y_DIGITs.shape[0]):
		save_image(y_DIGITs[i], 'gt' + str(i) + str(iter) + '.png')
		
	make_rand_DIGITs(rand_vecs, iters)
		
	print ("Saved")
	#model = load_model('shape_test1.h5')
